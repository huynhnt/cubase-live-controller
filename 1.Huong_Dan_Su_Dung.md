...
Ứng dụng tích hợp công nghệ phân tích và xác định Tone bài hát thông minh (**Smart Tone**) hoạt động trực tiếp bên trong ứng dụng:
* **Tự động nhận diện & phân tích:** Công nghệ Smart Tone tự động đọc tên bài hát đang phát trên trình duyệt (YouTube, Karaoke) hoặc phân tích tín hiệu âm thanh thực tế để xác định ngay lập tức nốt nhạc và giọng bài hát.
* **Đồng bộ trực tiếp sang Auto-Tune:** Khi dò ra kết quả (ví dụ: `Smart Tone: C# Minor`), ứng dụng phát lệnh gửi trực tiếp thông số **Key (CC 31)** và **Scale (CC 32)** sang plugin **Auto-Tune** trên Cubase và cập nhật ngay giao diện các phím nốt nhạc trên App mà không cần cắm thêm plugin Auto-Key hay thao tác cầu kỳ.

### 6. Bảng Âm Thanh Pha Trò (FX / Soundboard)
* Mở rộng bằng nút **FX ▾**.
...
4. Nhấn nút **Import** ở góc phải, tìm và chọn file cấu hình XML đã được xuất ra từ ứng dụng (Mở ứng dụng > Cài đặt > Xuất File Cấu Hình XML).
